// following suggestions by Christopher-Peterson in Stan forum
...
transformed parameters { 
  ...
  matrix[n_s, n_s] L_phylo = cholesky_decompose(
             rho * C + diag_matrix(rep_vector(1 - rho, n_s)));
...
model {
  ...
  for (n in 1:n_sites){
    for(s in 1:n_s){
      log_lambda[n,s] = dot_product( X[n,] , beta[s,]) + log(area[n]);
...
generated quantities{
  ...
  for (n in 1:n_sites){
    for(s in 1:n_s){
      N[n,s]= poisson_log_rng(dot_product( X[n,] , beta[s,]) + log(area[n])); 
      }
    }
